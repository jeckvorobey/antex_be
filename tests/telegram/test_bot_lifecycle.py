"""Тесты Telegram lifecycle и proxy-конфига внутри tests/telegram."""
...
@pytest.mark.asyncio
async def test_start_polling_retries_after_network_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_lifespan_uses_init_start_stop_flow(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_lifespan_skips_bot_start_without_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original_mode = app_main.settings.telegram_mode
    original_token = app_main.settings.telegram_bot_token
...
@pytest.mark.asyncio
async def test_webhook_accepts_raw_secret_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_webhook_requires_initialized_bot(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original_mode = telegram_router.settings.telegram_mode
    original_secret = telegram_router.settings.telegram_webhook_secret
    ...
    try:
        with pytest.raises(HTTPException):
            await telegram_router.telegram_webhook(DummyRequest())
    finally:
        monkeypatch.setattr(telegram_router.settings, "telegram_mode", original_mode)
        monkeypatch.setattr(telegram_router.settings, "telegram_webhook_secret", original_secret)
