...
class TestReferralBindingEdgeCases:
    ...
    async def test_bind_referral_chain(self, db_session: AsyncSession) -> None:
        ...
        user_a = await create_user(db_session, telegram_id=901010, referral_code="CHAIN_A")
        user_b = await create_user(db_session, telegram_id=901011)
        user_c = await create_user(db_session, telegram_id=901012)

        await service.bind_referral(db_session, user_b, "CHAIN_A")
    ...
    async def test_bind_same_code_by_two_users(
        self, db_session: AsyncSession
    ) -> None:
        """Два разных пользователя привязываются к одному рефереру."""
        service = ReferralService()
        referrer = await create_user(
            db_session, telegram_id=901020, referral_code="MULTIREF"
        )
        user1 = await create_user(db_session, telegram_id=901021)
        user2 = await create_user(db_session, telegram_id=901022)

        await service.bind_referral(db_session, user1, "MULTIREF")
        await service.bind_referral(db_session, user2, "MULTIREF")

        assert user1.referred_by == referrer.id
        assert user2.referred_by == referrer.id

        referrals = await service.get_referral_list(db_session, referrer)
        assert len(referrals) == 2

    async def test_referral_code_case_sensitive(
        self, db_session: AsyncSession
    ) -> None:
        """Реферальный код чувствителен к регистру."""
        service = ReferralService()
        await create_user(
            db_session, telegram_id=901030, referral_code="CaseSensitive"
        )
        user = await create_user(db_session, telegram_id=901031)

        with pytest.raises(AntExException) as exc_info:
            await service.bind_referral(db_session, user, "casesensitive")
        assert exc_info.value.code == "INVALID_REFERRAL_CODE"
...
class TestAexCalculationEdgeCases:
    ...
    async def test_referral_bonus_zero_order_amount(
        self, db_session: AsyncSession
    ) -> None:
        """Заказ на 0 → бонус 0 (не начисляется)."""
        referrer = await create_user(
            db_session, telegram_id=902001, referral_code="ZEROBON"
        )
        referred = await create_user(
            db_session, telegram_id=902002, referred_by=referrer.id
        )
    ...
    async def test_referral_bonus_small_amount_rounds_to_zero(
        ...
        referrer = await create_user(
            db_session, telegram_id=902003, referral_code="SMALLBN"
        )
        referred = await create_user(
            db_session, telegram_id=902004, referred_by=referrer.id
        )
    ...
    async def test_referral_bonus_fractional_amount(
        self, db_session: AsyncSession
    ) -> None:
        """Дробная сумма заказа → корректный расчёт."""
        referrer = await create_user(
            db_session, telegram_id=902005, referral_code="FRACBN"
        )
        referred = await create_user(
            db_session, telegram_id=902006, referred_by=referrer.id
        )
    ...
    async def test_referral_bonus_large_amount(
        self, db_session: AsyncSession
    ) -> None:
        """Огромная сумма заказа → корректный расчёт без переполнения."""
        referrer = await create_user(
            db_session, telegram_id=902007, referral_code="LARGEBN"
        )
        referred = await create_user(
            db_session, telegram_id=902008, referred_by=referrer.id
        )
    ...
    async def test_referral_bonus_with_very_high_rate(
        self, db_session: AsyncSession
    ) -> None:
        """Персональная ставка 10% → корректный расчёт."""
        referrer = await create_user(
            db_session, telegram_id=902009, referral_code="HIGHRT"
        )
        referred = await create_user(
            db_session, telegram_id=902010, referred_by=referrer.id
        )
...
class TestBalanceInvariant:
    ...
    async def test_multiple_operations_preserve_invariant(
        self, db_session: AsyncSession
    ) -> None:
...
class TestRatePriorityEdgeCases:
    ...
    async def test_personal_rate_takes_priority(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_delete_personal_falls_back_to_global(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_update_personal_rate_changes_effective(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_referral_bonus_uses_correct_rate(
        self, db_session: AsyncSession
    ) -> None:
        """Реферальный бонус использует эффективную ставку реферера, а не реферала."""  # noqa: RUF002
        referrer = await create_user(
            db_session, telegram_id=904010, referral_code="RATETST"
        )
        referred = await create_user(
            db_session, telegram_id=904011, referred_by=referrer.id
        )

        rate_service = AexRateService()
        # Персональная ставка для реферера = 5%
        await rate_service.set_personal_rate(
            db_session, referrer.id, Decimal("0.05")
        )
...
class TestCancellationCompensation:
    ...
    async def test_referral_bonus_reversal_on_cancel(
        self, db_session: AsyncSession
    ) -> None:
        """При отмене заказа бонус реферера списывается."""
        referrer = await create_user(
            db_session, telegram_id=905001, referral_code="CANCEL1"
        )
        referred = await create_user(
            db_session, telegram_id=905002, referred_by=referrer.id
        )
    ...
    async def test_no_reversal_if_no_referral_bonus(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_multiple_orders_cancel_one(
        self, db_session: AsyncSession
    ) -> None:
        """Two orders, cancel one - deduct only its bonus."""
        referrer = await create_user(
            db_session, telegram_id=905010, referral_code="MULTORD"
        )
        referred = await create_user(
            db_session, telegram_id=905011, referred_by=referrer.id
        )
...
class TestLedgerEntryIntegrity:
    ...
    async def test_credit_creates_correct_entry(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_debit_creates_negative_amount(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_hold_and_release_entries(
        self, db_session: AsyncSession
    ) -> None:
    ...
    async def test_operations_count_matches(
        self, db_session: AsyncSession
    ) -> None:
...
class TestReferralStatsEdgeCases:
    ...
    async def test_stats_after_multiple_bonuses(
        self, db_session: AsyncSession
    ) -> None:
        """Статистика учитывает все бонусы от разных рефералов."""
        referrer = await create_user(
            db_session, telegram_id=907001, referral_code="STATS1"
        )
        referred1 = await create_user(
            db_session, telegram_id=907002, referred_by=referrer.id
        )
        referred2 = await create_user(
            db_session, telegram_id=907003, referred_by=referrer.id
        )
    ...
    async def test_stats_with_mixed_entry_types(
        self, db_session: AsyncSession
    ) -> None:
        """Статистика считает только credit+referral, не другие типы."""
        referrer = await create_user(
            db_session, telegram_id=907010, referral_code="STATS2"
        )
